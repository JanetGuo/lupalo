...
var TDFriendSelector = (function(module, $) {
	...
	buildFriendSelector = function(callback) {
		...
		if (!FB) {
			alert( 'FaceBook SDK not initialised' )
		...
		FB.getLoginStatus(function(response) {
			alert( 'FaceBook get login status' )
			if (response.status === 'connected') {
					
				alert( 'FaceBook connected loading friends' )

				// Load Facebook friends
				FB.api('/me/friends?fields=id,name', function(response) {
					if (response.data) {
						log('TDFriendSelector - setting friends');
						setFriends(response.data);
						// Build the markup
						log('TDFriendSelector - building markup');
						buildMarkup();
						// Call the callback
						log('TDFriendSelector - calling callback');
